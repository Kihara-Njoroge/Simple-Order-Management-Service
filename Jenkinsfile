pipeline {
    ...
    stages {
        ...
        stage('Deploy Azure Minikube') {
            steps {
                sh '''
                    kubectl --kubeconfig $KUBE_CONFIG apply -f k8s/configmap.yaml
                    kubectl --kubeconfig $KUBE_CONFIG apply -f k8s/secret.yaml
                    kubectl --kubeconfig $KUBE_CONFIG set image deployment/order-management-api-deployment order-management-api=${DOCKER_IMAGE}
                '''
            }
        }
